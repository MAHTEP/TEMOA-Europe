...
MinEmissions = pd.read_sql("SELECT * FROM MinEmissions", conn)  # Loading the MinEmissions table from the .SQLite database
...
# Extracting the list of all indexes combinations for MinEmissions
indexes = list()
for i in range(0, len(MinEmissions)):
    indexes.append(MinEmissions.regions[i] + MinEmissions.emis_comm[i])
MinEmissions['indexes'] = indexes
indexes = list(dict.fromkeys(indexes))  # Removing duplicates

# Interpolating
for index_i in indexes:
    MinEmissions_i = MinEmissions[(MinEmissions['indexes'] == index_i)]
    MinEmissions_i = MinEmissions_i.sort_values(by=['periods'], ignore_index=True)
    for i in range(0, len(MinEmissions_i.periods)):
        if i < len(MinEmissions_i.periods) - 1:  # Interpolation
            # Extracting time periods involved in the interpolation
            time_periods_i = [x for x in time_periods if MinEmissions_i.periods[i] <= x < MinEmissions_i.periods[i+1]]
            for j in range(0, len(time_periods_i)):
                if j == 0:  # Only used for the first time period available (to avoid / 0 in the linear interpolation equation)
                    regions.append(MinEmissions_i.regions[i])
                    periods.append(time_periods_i[j])
                    emis_comm.append(MinEmissions_i.emis_comm[i])
                    min_emis.append(float(MinEmissions_i.min_emis[i]))
                    min_emis_units.append(MinEmissions_i.min_emis_units[i])
                    min_emis_notes.append(MinEmissions_i.min_emis_notes[i])
                else:  # Linear interpolation for intermediate time periods
                    regions.append(MinEmissions_i.regions[i])
                    periods.append(time_periods_i[j])
                    emis_comm.append(MinEmissions_i.emis_comm[i])
                    min_emis.append(float(MinEmissions_i.min_emis[i] +
                                            (MinEmissions_i.min_emis[i + 1] - MinEmissions_i.min_emis[i]) *
                                            (time_periods_i[j] - MinEmissions_i.periods[i]) /
                                            (MinEmissions_i.periods[i + 1] - MinEmissions_i.periods[i])))
                    min_emis_units.append(MinEmissions_i.min_emis_units[i])
                    min_emis_notes.append(MinEmissions_i.min_emis_notes[i])
        else:  # Extrapolation
            # Extracting time periods involved in the extrapolation
            time_periods_i = [x for x in time_periods if x == MinEmissions_i.periods[i]]
            for j in range(0, len(time_periods_i)):
                regions.append(MinEmissions_i.regions[i])
                periods.append(time_periods_i[j])
                emis_comm.append(MinEmissions_i.emis_comm[i])
...
# Converting lists into a DataFrame
MinEmissions = pd.DataFrame(
    {
        "regions": pd.Series(regions, dtype='str'),
        "periods": pd.Series(periods, dtype='int'),
        "emis_comm": pd.Series(emis_comm, dtype='str'),
        "min_emis": pd.Series(min_emis, dtype='float'),
        "min_emis_units": pd.Series(min_emis_units, dtype='str'),
        "min_emis_notes": pd.Series(min_emis_notes, dtype='str')
    }
)

if save_tosql['MinEmissions']:
    MinEmissions.to_sql('MinEmissions', conn, index=False, if_exists='replace')

if print_outcome['MinEmissions']:
    pd.set_option('display.max_rows', len(MinEmissions))
    pd.set_option('display.max_columns', 10)
    print("\nMinEmissions DataFrame\n\n", MinEmissions)
    pd.reset_option('display.max_rows')
    pd.reset_option('display.max_columns')
...
print_i = print_i + 1
if print_status:
    print("{:>1} {:>2} {:>1} {:>2} {:>1} {:>50} {:>6} {:>1}".format('[', print_i, '/', len(print_outcome), ']', 'MinEmissions interpolated.',
                                                                    np.format_float_positional(abs(end_time - start_time), 2), 's'))
