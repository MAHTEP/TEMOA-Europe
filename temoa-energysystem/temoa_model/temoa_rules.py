...
"""
    cap_avail = sum(
        value(M.ProcessLifeFrac[r, p, t, S_v]) * M.V_Capacity[r, t, S_v] * M.CapacityReduction[r, t, p]
...
refers to the :code:`MaxGenGroupLimit` parameter.
"""

    activity_p = sum(
        M.V_FlowOut[r, p, s, d, S_i, S_t, S_v, S_o] * M.MaxGenGroupWeight[r, S_t, g]
    ...
    activity_p_annual = sum(
        M.V_FlowOutAnnual[r, p, S_i, S_t, S_v, S_o] * M.MaxGenGroupWeight[r, S_t, g]
        for r in M.RegionalIndices
...
"""
    inp = sum(
        M.V_FlowOut[r, p, s, d, i, S_t, S_v, S_o] * M.MinInputGroupWeight[r, S_t, g] / value(M.Efficiency[r, i, S_t, S_v, S_o])
    ...
    inp_annual = sum(
        M.V_FlowOutAnnual[r, p, i, S_t, S_v, S_o] * M.MinInputGroupWeight[r, S_t, g] / value(M.Efficiency[r, i, S_t, S_v, S_o])
        for S_r, S_p, S_t, S_v in M.activeActivity_rptv
        if S_r == r
        if S_p == p
        if S_t in M.tech_groups
    ...
    total_inp = sum(
        M.V_FlowOut[r, p, s, d, S_i, S_t, S_v, S_o] * M.MinInputGroupWeight[r, S_t, g] / value(M.Efficiency[r, S_i, S_t, S_v, S_o])
        for S_r, S_p, S_t, S_v in M.activeActivity_rptv
        if S_r == r
        if S_p == p
        if S_t in M.tech_groups
    ...
    total_inp_annual = sum(
        M.V_FlowOutAnnual[r, p, S_i, S_t, S_v, S_o] * M.MinInputGroupWeight[r, S_t, g] / value(M.Efficiency[r, S_i, S_t, S_v, S_o])
        for S_r, S_p, S_t, S_v in M.activeActivity_rptv
        if S_r == r
        if S_p == p
        if S_t in M.tech_groups
...
"""
    inp = sum(
        M.V_FlowOut[r, p, s, d, i, S_t, S_v, S_o] * M.MaxInputGroupWeight[r, S_t, g] / value(M.Efficiency[r, i, S_t, S_v, S_o])
        for S_r, S_p, S_t, S_v in M.activeActivity_rptv
        if S_r == r
        if S_p == p
        if S_t in M.tech_groups
    ...
    inp_annual = sum(
        M.V_FlowOutAnnual[r, p, i, S_t, S_v, S_o] * M.MaxInputGroupWeight[r, S_t, g] / value(M.Efficiency[r, i, S_t, S_v, S_o])
        for S_r, S_p, S_t, S_v in M.activeActivity_rptv
        if S_r == r
        if S_p == p
        if S_t in M.tech_groups
    ...
    total_inp = sum(
        M.V_FlowOut[r, p, s, d, S_i, S_t, S_v, S_o] * M.MaxInputGroupWeight[r, S_t, g] / value(M.Efficiency[r, S_i, S_t, S_v, S_o])
        for S_r, S_p, S_t, S_v in M.activeActivity_rptv
        if S_r == r
        if S_p == p
        if S_t in M.tech_groups
    ...
    total_inp_annual = sum(
        M.V_FlowOutAnnual[r, p, S_i, S_t, S_v, S_o] * M.MaxInputGroupWeight[r, S_t, g] / value(M.Efficiency[r, S_i, S_t, S_v, S_o])
        for S_r, S_p, S_t, S_v in M.activeActivity_rptv
        if S_r == r
        if S_p == p
        if S_t in M.tech_groups
...
"""
    outp = sum(
        M.V_FlowOut[r, p, s, d, S_i, S_t, S_v, o] * M.MaxOutputGroupWeight[r, S_t, g]
        for S_r, S_p, S_t, S_v in M.activeActivity_rptv
        if S_r == r
        if S_p == p
        if S_t in M.tech_groups
    ...
    outp_annual = sum(
        M.V_FlowOutAnnual[r, p, S_i, S_t, S_v, o] * M.MaxOutputGroupWeight[r, S_t, g]
        for S_r, S_p, S_t, S_v in M.activeActivity_rptv
        if S_r == r
        if S_p == p
        if S_t in M.tech_groups
    ...
    total_outp = sum(
        M.V_FlowOut[r, p, s, d, S_i, S_t, S_v, S_o] * M.MaxOutputGroupWeight[r, S_t, g]
        for S_r, S_p, S_t, S_v in M.activeActivity_rptv
        if S_r == r
        if S_p == p
        if S_t in M.tech_groups
    ...
    total_outp_annual = sum(
        M.V_FlowOutAnnual[r, p, S_i, S_t, S_v, S_o] * M.MaxOutputGroupWeight[r, S_t, g]
        for S_r, S_p, S_t, S_v in M.activeActivity_rptv
        if S_r == r
        if S_p == p
        if S_t in M.tech_groups
