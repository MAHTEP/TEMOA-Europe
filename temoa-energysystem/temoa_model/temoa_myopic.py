...
def myopic_db_generator_solver ( self ):
    ...
    for i in range(N-1,len(time_periods)-1):
        ...
        if i!=(N-1):
            ...
            cur_org.execute("DELETE FROM Output_V_Capacity WHERE scenario=" + "'" + str(
                self.options.scenario) + "' AND t_periods>" + str(time_periods[i - N][0]))
            cur_org.execute("DELETE FROM Output_V_NewCapacity WHERE scenario=" + "'" + str(
                self.options.scenario) + "' AND vintage>" + str(time_periods[i - N][0]))
            cur_org.execute("DELETE FROM Output_V_RetiredCapacity WHERE scenario=" + "'" + str(
                self.options.scenario) + "' AND t_periods>" + str(time_periods[i - N][0]))
